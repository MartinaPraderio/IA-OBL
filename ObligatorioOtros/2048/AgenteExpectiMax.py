...
WEIGHT_MATRIX2 = np.array([[1,  1,  2,    4], 
                          [1,  2,  8,    256],
                          [2,  16, 128,  512],
                          [32, 64, 1024, 2048]])
...
WEIGHT_MATRIX3 = np.array([[1,  1,  2,    3], 
                          [1,  2,  3, 8],
                          [12, 11, 10,  9],
                          [13, 14, 15, 16]])

class AgentExpextiMax(Agent):
    ...
    def play(self, board:GameBoard):
        self.idx = 0
        accion, value = self.expectimax(board, self.player, self.max_depth)
        return accion
    ...
    def heuristic_utility(self, board: GameBoard):
        ...
        heuristic = h_available_cell + h_available_moves + 0.05 * h_get_max + 0.0001 * h_weighted_sum
        return heuristic

    def expectimax(self, board: GameBoard, player: int,  depth: int):    
        ...
        if player != self.player: # expecty
            minimax_list = [(action, self.expectimax(child_node, self.player, depth-1)[1]) for (action, child_node) in action_nodes]
            chosen_action = None
            value = np.mean(list(v for a, v in minimax_list))
            ...
            minimax_list = [(action, self.expectimax(child_node, self.other_player, depth-1)[1]) for (action, child_node) in action_nodes]
